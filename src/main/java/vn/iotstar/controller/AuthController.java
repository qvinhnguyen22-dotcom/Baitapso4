...
@WebServlet(urlPatterns = {
    "/register", "/verify-otp", "/login", "/forgot-password", "/reset-password", "/logout"
})
public class AuthController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (path) {
            ...
            case "/verify-otp" -> verifyRegistration(request, response);
    ...
    private void register(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isBlank(fullname) || isBlank(email) || isBlank(username) || isBlank(password)
            ...
            showError(request, response, "/views/register.jsp", "Thông tin không hợp lệ hoặc mật khẩu không khớp.");
            return;
        }
        if (userDao.findByEmail(email) != null || userDao.findByUsername(username.trim()) != null) {
            showError(request, response, "/views/register.jsp", "Email hoặc tên đăng nhập đã tồn tại.");
            return;
        }
    ...
    private void verifyRegistration(HttpServletRequest request, HttpServletResponse response)
            throws IOException, ServletException {
        HttpSession session = request.getSession();
        ...
        if (user == null || !isValidOtp(session, "registrationOtpCreatedAt", user, otp)) {
            showError(request, response, "/views/verify-otp.jsp", "Mã OTP không đúng hoặc đã hết hạn.");
            return;
        }
    ...
    private void login(HttpServletRequest request, HttpServletResponse response)
        ...
        String username = request.getParameter("username");
        String password = request.getParameter("password");
        User user = isBlank(username) ? null : userDao.findByUsername(username.trim());
        if (user == null || !user.isStatus() || !user.getPassword().equals(password)) {
            String message = user != null && !user.isStatus()
                    ? "Tài khoản chưa được kích hoạt. Hãy xác thực OTP trước."
                    : "Tên đăng nhập hoặc mật khẩu không đúng.";
            showError(request, response, "/views/login.jsp", message);
            return;
        }
    ...
    private void forgotPassword(HttpServletRequest request, HttpServletResponse response)
            throws IOException, ServletException {
        String email = normalizeEmail(request.getParameter("email"));
        User user = isBlank(email) ? null : userDao.findByEmail(email);
        if (user == null) {
            showError(request, response, "/views/forgot-password.jsp", "Email không tồn tại.");
            return;
        }
    ...
    private void resetPassword(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user == null || !isValidOtp(session, "resetOtpCreatedAt", user, otp)
                || isBlank(password) || !password.equals(confirmPassword)) {
            showError(request, response, "/views/reset-password.jsp", "OTP không đúng, đã hết hạn hoặc mật khẩu không khớp.");
            return;
        }
    ...
    private boolean isValidOtp(HttpSession session, String createdAtKey, User user, String otp) {
        ...
        return !isBlank(otp) && otp.equals(user.getCode()) && createdAt instanceof Long
                && System.currentTimeMillis() - (Long) createdAt <= OTP_VALIDITY_MILLIS;
    }

    private void showError(HttpServletRequest request, HttpServletResponse response, String view, String error)
            throws ServletException, IOException {
